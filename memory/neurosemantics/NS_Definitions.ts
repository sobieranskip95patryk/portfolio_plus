...
export interface NeuroSemanticWeight {
    area: string;
    weight: number;
...
export const NS_WEIGHTS: Record<string, NeuroSemanticWeight> = {
    // ================= BLOK WZBUDZENIA =================
...
export const NSUtils = {
     ...
     */
    getHighestWeightElements: (threshold: number = 0.8): string[] => {
        return Object.entries(NS_WEIGHTS)
     ...
     */
    getMostStableElements: (threshold: number = 0.15): string[] => {
        return Object.entries(NS_WEIGHTS)
     ...
     */
    getElementsByBrainArea: (): Record<string, string[]> => {
        const areas: Record<string, string[]> = {};
        
        Object.entries(NS_WEIGHTS).forEach(([name, data]) => {
            const primaryArea = data.area.split('/')[0]; // Bierz pierwszy obszar
            if (!areas[primaryArea]) areas[primaryArea] = [];
     ...
     */
    calculateAverageWeight: (elements: string[]): number => {
        if (elements.length === 0) return 0;
        
        const totalWeight = elements.reduce((sum, element) => {
            return sum + (NS_WEIGHTS[element]?.weight || 0);
        }, 0);
