...
import { applyEmotionalBoost } from '../neurosemantics/emotionalBoost';
 ...
 */
export function startNSF_Flowmeter(relationalGraph: RelationalGraph): NodeJS.Timeout {
    ...
    const sekundnikTimer = setInterval(() => {
        ...
        try {
            ...
            decayScheduler.applyDecay(relationalGraph);
    ...
    }, NSF_CONSTANTS.SEKUNDNIK_INTERVAL);
 ...
 */
function processContextExcitation(relationalGraph: RelationalGraph, query: string | null): SenseAtom[] {
    const activatedAtoms: SenseAtom[] = [];
 ...
 */
function applyPFC_Logic(activatedAtoms: SenseAtom[], relationalGraph: RelationalGraph): void {
    if (activatedAtoms.length === 0) return;
    ...
    const altruisticElements = activatedAtoms.filter(atom => 
        atom.detectedPierwiastki?.includes('POSWIECENIE') ||
    ...
    const negativeElements = activatedAtoms.filter(atom =>
        atom.detectedPierwiastki?.includes('BRAK_WIARY_I_NADZIEI') ||
 ...
 */
function detectPierwiastkiInQuery(query: string): string[] {
 ...
 */
function activateTopWeightAtoms(relationalGraph: RelationalGraph, count: number): SenseAtom[] {
 ...
 */
function checkLogicCoherence(atoms: SenseAtom[]): number {
    if (atoms.length < 2) return 1.0;
    ...
    for (let i = 0; i < atoms.length - 1; i++) {
        for (let j = i + 1; j < atoms.length; j++) {
            ...
            if (atom1.detectedPierwiastki && atom2.detectedPierwiastki) {
                const overlap = atom1.detectedPierwiastki.filter(p => 
                    atom2.detectedPierwiastki!.includes(p)
                ).length;
                
                const maxElements = Math.max(
                    atom1.detectedPierwiastki.length, 
                    atom2.detectedPierwiastki.length
                );
 ...
 */
function boostAltruisticConnections(altruisticAtoms: SenseAtom[], relationalGraph: RelationalGraph): void {
 ...
 */
function isolateNegativeStates(negativeAtoms: SenseAtom[], relationalGraph: RelationalGraph): void {
 ...
 */
function generateInsights(atoms: SenseAtom[], relationalGraph: RelationalGraph): void {
 ...
 */
function updateNSFState(relationalGraph: RelationalGraph, activatedAtoms: SenseAtom[], cycleStart: number): void {
    const cycleTime = performance.now() - cycleStart;
    ...
    nsfState.performance.consolidatedAtoms = 
        relationalGraph.getAllAtoms().filter(atom => atom.senseWeight > NSF_CONSTANTS.CONSOLIDATION_THRESHOLD).length;
 ...
 * Zatrzymanie NSF
 */
export function stopNSF_Flowmeter(timer: NodeJS.Timeout): void {
    clearInterval(timer);
 ...
 */
export function setCurrentQuery(query: string | null): void {
 ...
 */
export function getNSFState(): NSFState {
    return { ...nsfState };
}
