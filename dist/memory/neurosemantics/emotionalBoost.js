...
exports.applyEmotionalBoost = applyEmotionalBoost;
exports.isHighEmotionalCharge = isHighEmotionalCharge;
exports.applyOxidativeStress = applyOxidativeStress;
exports.diagnoseAmygdalaState = diagnoseAmygdalaState;
const NS_Definitions_1 = require("./NS_Definitions");
/**
 * Wzmacniacz emocjonalny - główna funkcja symulująca Ciało Migdałowate
 */
function applyEmotionalBoost(senseAtom, detectedPierwiastki) {
 ...
 */
function calculateEmotionalBoost(pierwiastki) {
    let totalWeight = 0;
    ...
    const highEmotionalElements = processedElements.filter(p => {
        const data = NS_Definitions_1.NS_WEIGHTS[p];
        return data && (p.includes('MILOSCI') ||
    ...
    return {
        ...
        finalDecayRate: Math.max(minDecayModifier, NS_Definitions_1.NSF_CONSTANTS.MIN_SENSE_WEIGHT / 10), // Minimum decay
        activationLevel,
        stabilization,
        processedElements,
 ...
 */
function calculateEmotionalIntensity(elements) {
 ...
 */
function generateAmygdalaResponse(level, elements) {
    const responses = {
        'MINIMAL': {
            priority: 'LOW',
 ...
 */
function isHighEmotionalCharge(pierwiastek) {
    const neuroData = NS_Definitions_1.NS_WEIGHTS[pierwiastek];
    if (!neuroData)
        return false;
 ...
 */
function applyOxidativeStress(senseAtom, totalActivatedAtoms) {
    if (totalActivatedAtoms > NS_Definitions_1.NSF_CONSTANTS.MAX_ACTIVE_ATOMS * 0.8) {
        const stressFactor = Math.min(totalActivatedAtoms / NS_Definitions_1.NSF_CONSTANTS.MAX_ACTIVE_ATOMS, 2.0);
 ...
 */
function diagnoseAmygdalaState(activeAtoms) {
