...
exports.NSFIntegrationTest = void 0;
exports.runNSFTest = runNSFTest;
...
const NS_Definitions_1 = require("./memory/neurosemantics/NS_Definitions");
/**
 * Klasa testowa dla NSF
 */
class NSFIntegrationTest {
    constructor() {
        this.nsfTimer = null;
    ...
    async setupTestData() {
        ...
        const testAtoms = [
            // Kategoria MIŁOŚCI
            new SenseAtom_1.SenseAtom('love_001', 'Pierwsza miłość', 2.5, ['ALGORYTM_ZAKOCHANIA', 'PIERWIASTEK_MILOSCI']),
            new SenseAtom_1.SenseAtom('love_002', 'Miłość bezwarunkowa', 3.0, ['MILOSC_BEZWARUNKOWA', 'POSWIECENIE']),
            // Kategoria POZNANIA
            new SenseAtom_1.SenseAtom('cognition_001', 'Rozwiązanie problemu', 1.8, ['SZCZYPTA_INTELIGENCJI', 'ZROZUMIENIE']),
            new SenseAtom_1.SenseAtom('cognition_002', 'Kreatywny pomysł', 2.2, ['POMYSL', 'ENERGIA_TWORCZA']),
            // Kategoria EMOCJONALNA
            new SenseAtom_1.SenseAtom('emotion_001', 'Namiętność', 2.8, ['NAMIENTNOSC_POZADANIA', 'POZADANIE_SZCZYPTA']),
            new SenseAtom_1.SenseAtom('emotion_002', 'Nadzieja na lepsze', 1.9, ['NADZIEJA_WYTRWALOSCI', 'WIARA_JAKO_TAKA']),
            // Kategoria TRANSCENDENCJI
            new SenseAtom_1.SenseAtom('transcend_001', 'Moment oświecenia', 3.2, ['TRANSCENDENCJA_GRANIC', 'JEDNOSC_BYTU']),
            new SenseAtom_1.SenseAtom('transcend_002', 'Iskra życia', 4.0, ['ISKRA_ZYCIA', 'SEKUNDNIK']),
            // Kategoria TRUDNOŚCI
            new SenseAtom_1.SenseAtom('struggle_001', 'Moment zwątpienia', 1.2, ['BRAK_WIARY_I_NADZIEI']),
            new SenseAtom_1.SenseAtom('struggle_002', 'Nieprzejednana walka', 1.5, ['NIEUGIETA_BEZSILNOSC']),
        ];
    ...
    async simulateUserInteractions() {
        ...
        for (let i = 0; i < testQueries.length; i++) {
            ...
            (0, excitationEngine_1.setCurrentQuery)(query);
    ...
    analyzeQueryResults(query) {
        ...
        if (activeAtoms.length > 0) {
            const topAtom = activeAtoms.reduce((prev, current) => prev.senseWeight > current.senseWeight ? prev : current);
            console.log(`   🏆 Najsilniejszy atom: ${topAtom.label} (waga: ${topAtom.senseWeight.toFixed(2)})`);
            // Diagnoza stanu Ciała Migdałowatego
            const amygdalaState = (0, emotionalBoost_1.diagnoseAmygdalaState)(activeAtoms);
    ...
    async stopAndAnalyze() {
        ...
        const finalState = (0, excitationEngine_1.getNSFState)();
     ...
     */
    generateTestReport(nsfState, decayStats, graphStats) {
        ...
        const highWeightElements = NS_Definitions_1.NSUtils.getHighestWeightElements(0.8);
        const stableElements = NS_Definitions_1.NSUtils.getMostStableElements(0.15);
     ...
     */
    getSuccessRating(score) {
        if (score >= 90)
            return '🌟 WYBITNY - NSF działa perfekcyjnie!';
        if (score >= 80)
            return '🚀 DOSKONAŁY - Wszystkie systemy operacyjne!';
        if (score >= 70)
            return '✨ BARDZO DOBRY - NSF w pełnej gotowości!';
        if (score >= 60)
            return '👍 DOBRY - Podstawowe funkcje działają!';
        if (score >= 50)
            return '⚠️ ZADOWALAJĄCY - Wymaga optymalizacji';
    ...
    async runQuickTest() {
        ...
        const testAtom = new SenseAtom_1.SenseAtom('quick_test', 'Test NSF', 1.0, ['ISKRA_ZYCIA']);
        this.relationalGraph.addAtom(testAtom);
        // Zastosuj wzmocnienie
        (0, emotionalBoost_1.applyEmotionalBoost)(testAtom, ['ISKRA_ZYCIA']);
        // Jeden cykl zaniku
        decayScheduler_1.decayScheduler.applyDecay(this.relationalGraph);
        console.log(`✅ Szybki test zakończony - atom: ${testAtom.label}, waga: ${testAtom.senseWeight.toFixed(3)}\n`);
    }
}
exports.NSFIntegrationTest = NSFIntegrationTest;
// ================= FUNKCJA GŁÓWNA =================
/**
 * Uruchamia test NSF
 */
async function runNSFTest(fullTest = true) {
