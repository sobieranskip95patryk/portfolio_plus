...
import { NS_WEIGHTS, NSF_CONSTANTS } from './memory/neurosemantics/NS_Definitions';
import { startNSF_Flowmeter, stopNSF_Flowmeter, getNSFState, setCurrentQuery } from './memory/logic/excitationEngine';
import { decayScheduler } from './memory/logic/decayScheduler';
import { diagnoseAmygdalaState } from './memory/neurosemantics/emotionalBoost';

/**
 * Stan meta-poznawczy systemu NSF
 */
interface MetaCognitiveState {
    // === SAMOŚWIADOMOŚĆ ===
    selfAwarenessLevel: number;           // 0.0 - 1.0
    currentFocus: string | null;          // Obecny fokus uwagi
    mentalEnergy: number;                 // 0.0 - 1.0 energia mentalna
    cognitiveLoad: number;                // 0.0 - 1.0 obciążenie poznawcze
    
    // === MONITOROWANIE WYDAJNOŚCI ===
    performanceMetrics: {
        averageResponseTime: number;      // Średni czas odpowiedzi
        memoryEfficiency: number;         // Efektywność pamięci
        emotionalStability: number;       // Stabilność emocjonalna
        logicalCoherence: number;         // Spójność logiczna
        creativityIndex: number;          // Indeks kreatywności
    };
    
    // === AUTOOPTYMALIZACJA ===
    adaptiveParameters: {
        decayRateModifier: number;        // Modyfikator współczynnika zaniku
        emotionalSensitivity: number;     // Wrażliwość emocjonalna
        logicalRigor: number;             // Rygor logiczny
        creativityBoost: number;          // Wzmocnienie kreatywności
        attentionThreshold: number;       // Próg uwagi
    };
    
    // === HISTORIA I TRENDY ===
    performanceHistory: PerformanceSnapshot[];
    learningProgress: number;             // Postęp w uczeniu się
    adaptationScore: number;              // Ocena adaptacji
    
    // === CELE I PRIORYTETY ===
    currentGoals: string[];               // Bieżące cele
    priorityWeights: Map<string, number>; // Wagi priorytetów
    valueAlignment: number;               // Zgodność z wartościami
}

/**
 * Migawka wydajności w określonym momencie
 */
 ...
 */
export class MetaCognitiveNSFEngine {
    private relationalGraph: RelationalGraph;
    private nsfTimer: NodeJS.Timeout | null = null;
    private metaState: MetaCognitiveState;
    private introspectionTimer: NodeJS.Timeout | null = null;
    private optimizationInterval: number = 5000; // 5s
    private isActive: boolean = false;

    constructor(relationalGraph: RelationalGraph) {
     ...
     */
    public async startIntegratedSystem(): Promise<void> {
     ...
     */
    private performSelfIntrospection(): void {
     ...
     */
    private analyzeCurrentPerformance(nsfState: any, graphStats: any, decayStats: any): PerformanceSnapshot {
     ...
     */
    private updateSelfAwareness(performance: PerformanceSnapshot): void {
    ...
    private analyzeEmotionalState(): void {
        ...
        if (emotionalAtoms.length > 0) {
            const amygdalaState = diagnoseAmygdalaState(emotionalAtoms);
     ...
     */
    private analyzeCognitiveLoad(nsfState: any): void {
     ...
     */
    private recordPerformanceSnapshot(snapshot: PerformanceSnapshot): void {
     ...
     */
    private optimizeDecayParameters(trends: any): void {
        if (!trends) return;
     ...
     */
    private optimizeEmotionalSensitivity(trends: any): void {
        if (!trends) return;
     ...
     */
    private optimizeAttentionThreshold(trends: any): void {
        if (!trends) return;
     ...
     */
    private updateGoalsBasedOnPerformance(trends: any): void {
     ...
     */
    private generateIntrospectionReport(): void {
     ...
     */
    private reportOptimizationResults(): void {
    ...
    private calculateEmotionalActivation(): number {
    ...
    private calculateLogicalProcessing(): number {
     ...
     */
    public async stopIntegratedSystem(): Promise<void> {
     ...
     */
    private adaptParametersToFocus(focus: string): void {
        switch (focus) {
            case 'creativity':
